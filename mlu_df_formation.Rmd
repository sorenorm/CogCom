...
filelist <- list.files(path="/CogSci/CogCom/Week 3/Class/CogCom/data", full.names = "TRUE")
...
```{R}
sarah_data <- map_df(filelist,function(x) read.CLAN.file(x))
```
...
sarah_data_select<- select(sarah_data, Speaker, Gloss, Date)
...
write.csv(sarah_data_select, "sarah_data_select.csv")
...
```{R}
sarah_data_select <- read.csv("sarah_data_select.csv", header = T)
```
...
```{R}
class(sarah_data_select$Speaker)

class(sarah_data_select$Gloss)

class(sarah_data_select$Date)
...
```{R}
sarah_data_select$Date <- anytime::anydate(sarah_data_select$Date)
...
```{R}
MCS <- filter(sarah_data_select, sarah_data_select$Speaker=="MOT" | sarah_data_select$Speaker=="CHI")
...
```{R}
MCS$GlossClear <- str_replace_all(MCS$Gloss, pattern="[.?+]", replacement="")
...
MCS$WordCount <- stri_count_words(MCS$GlossClear)
...
```{R}
MCS$Speaker <- as.factor(MCS$Speaker)

class(MCS$Speaker)


G_MCS <- group_by(MCS, Date, Speaker)
...
I have already given you the structure of how summarise can be used to create a new dataframe with the desired values. 
4.4) Replace quotated parts in order to create a data frame that contains 
I) number of words 
II)utterances and 
...
mlu_df <- summarise(G_MCS, word_sum = sum(G_MCS$WordCount),
                                         utterances = length(G_MCS$Speaker),
